...
sub reconstruct_master_login {
	...
	if (exists $args->{password}) {
		...
		my $chain = pack('C24', (0x40, 0x21, 0xA9, 0x06, 0x5B, 0xA1, 0xB8, 0x36, 0xD5, 0x03, 0x2E, 0x51, 0x06, 0x00, 0x12, 0x34, 0x40, 0x21, 0xA9, 0x06, 0x5B, 0xA1, 0xB8, 0x36));
		
		for (my $i=0; $i < length($args->{password}); $i++) {
			substr($password, $i, 1) = substr($args->{password}, $i, 1);
		}
